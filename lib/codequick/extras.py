...
class viewModeSelecter(Base):
	...
	def __init__(self):
		...
		try: database = self.dict_storage(skincodePath)
		...
		if newMode is not None: self.set_setting("%s.%s.view" % (self.skinID, self.mode), newMode)
	...
	def display(self, skinCodes):
		...
		try: currentMode = self.get_setting("%s.%s.view" % (self.skinID, self.mode))
		except ValueError: currentMode = ""
		
		# Create list of item to show to user
		reference = [None]
		showList = [self.get_local_string("Default")]
		...
		if currentMode and currentMode not in skinCodes.values(): custom = u"[B]-%s (%i)[/B]" % (self.get_local_string("Custom"), currentMode)
		else: custom = self.get_local_string("Custom")
	...
	def askForViewID(self, currentMode):
		...
		dialog = xbmcgui.Dialog()
		ret = dialog.numeric(0, self.get_local_string("Enter_number"), str(currentMode))
...
class SavedSearches(VirtualFS):
	...
	def list_terms(self):
		...
		strRemove = self.get_local_string("Remove")
		queryCx = self.copy()
		queryLi = self.copy()
