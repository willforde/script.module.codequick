...
class YTPlaylist(VirtualFS):
    ...
    def start(self):
        ...
        if "playlistid" in self:
            return Gdata.playlist(playlistID=self[u"playlistid"])
        else:
            return Gdata.playlist(contentID=self[u"contentid"])
...
class YTPlaylists(VirtualFS):
    ...
    def start(self):
        if "channelid" in self:
            return self._initiate(channelID=self[u"channelid"])
        else:
            return self._initiate(contentID=self[u"contentid"])
...
class APIControl(object):
    ...
    def _check_contentID(self, contentID, returnPlaylistID=True):
        ...
        if contentID[:2] == "UU" or contentID[:2] == "PL":
            if returnPlaylistID:
                ...
                raise ValueError("ContentID is not a valid ID for wanted type: %s" % contentID)

        # If channel id then either return it or fetch the playlistID for channel
        elif contentID[:2] == "UC":
            if not returnPlaylistID:
                return contentID
            else:
    ...
    def playlists(self, contentID=None, channelID=None):
        ...
        # Fetch channel ID
        if contentID:
            channelID = self._check_contentID(contentID, returnPlaylistID=False)
        elif not channelID:
        ...
        if channelID in channel_cache:
            fanart = channel_cache[channelID]["fanart"]
        else:
            fanart = None
    ...
    def playlist(self, contentID=None, playlistID=None):
        ...
        # Connect to API server and Fetch response
        if contentID:
            playlistID = self._check_contentID(contentID, returnPlaylistID=True)
        elif not playlistID:
        ...
        videoList = [(item[u"snippet"][u"channelId"], item[u"snippet"][u"resourceId"][u"videoId"]) for item in
                     feed[u"items"] if item[u"status"][u"privacyStatus"] == u"public"]
        return self._videos(videoList, feed.get(u"nextPageToken"))
    ...
    def _videos(self, videoIDs, pagetoken=None):
        ...
        for channelID, videoId in videoIDs:
            ...
            if isHD >= 1 and contentDetails[u"definition"] == u"hd":
                item.addStreamInfo(isHD)
            else:
            ...
            if durationStr:
                ...
                for time, timeType in durationStr:
                    if timeType == "H":
                        duration += (localInt(time) * 3600)
                    elif timeType == "M":
                        duration += (localInt(time) * 60)
                    elif timeType == "S":
                        duration += (localInt(time))
            ...
            if len(channelIDs) > 1: item.menu_update(YTPlaylist, "Go to: %s" % snippet[u"channelTitle"],
                                                     contentID=channelID)
...
class API(object):
    ...
    def __init__(self, req_session, lang="en", maxResults="50", prettyPrint="false",
    ...
    def _connect_v3(self, type, params, max_age=None):
        ...
        if "id" in params and (isinstance(params["id"], list) or isinstance(params["id"], set)): params[
            "id"] = u",".join(params["id"])
    ...
    def Channels(self, id=None, forUsername=None):
        ...
        params[
            "fields"] = u"items(id,brandingSettings/image/bannerTvMediumImageUrl,contentDetails/relatedPlaylists/uploads,snippet/localized)"
    ...
    def Videos(self, id):
        ...
        params = self.default_params.copy()
        params[
            "fields"] = u"items(id,snippet(publishedAt,channelId,thumbnails/medium/url,channelTitle,categoryId,localized),contentDetails(duration,definition),statistics/viewCount)"
    ...
    def Playlists(self, channelID):
        ...
        params = self.default_params.copy()
        params[
            "fields"] = u"nextPageToken,items(id,contentDetails/itemCount,snippet(publishedAt,localized,thumbnails/medium/url))"
