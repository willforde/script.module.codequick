...
class RequestRes(object):
	def __init__(self, response):
		...
		try:
			print "# len compressed", len(content)
			if contentEncoding and "gzip" in contentEncoding: content = zlib.decompress(content, 16+zlib.MAX_WBITS)
			elif contentEncoding and "deflate" in contentEncoding: content = zlib.decompress(content)
		
		except zlib.error as e:
			print "error: %s" % e
		
		else:
			print "# len uncompressed", len(content)
			self.content = content
	
	@property
	def text(self):
		if self.__text: return self.__text
		else:
			# Convert html to unicode
			try: unicodeData = unicode(self.content, self.encoding if self.encoding else "utf8")
			except: unicodeData = unicode(self.content, "iso-8859-1")
...
class Session(object):
	...
	def request(self, method, url, params=None, data=None, headers=None, timeout=None):
		...
		if self.__opener: opener = self.__opener
		else: self.__opener = opener = urllib2.build_opener(*self.handleList)
...
class CacheAdapter(urllib2.BaseHandler):
	...
	def http_response(self, request, response):
		if self.from_cache: return response
...
class CacheHandler(object):
	...
	def __init__(self, cache_dir, hash, preload_content=False):
		...
		elif preload_content is True: self.load()
	...
	def delete(path):
		...
		try: os.remove(path)
		except OSError: logger.debug("Cache Error: Unable to delete cache from disk")
		self.close()
	...
	def response(self):
		...
		return functools.partial(HTTPResponse, self._response["body"], self._response["headers"], self._response["status"], self._response["reason"])
	...
	def fresh(self, max_age):
		...
		else: return max_age < 0 or self._response["status"] == 301 or (time.time() - self._response.get("timestamp", 0)) < max_age * 60
	...
	def update_cache(self, response=None):
		...
		try: jsonSerial = json.dumps(response, ensure_ascii=True, indent=4, separators=(",", ":"))
		except TypeError:
			logger.debug("Cache Error: Failed to serialize the response using json")
			return None
		
		# Compress the json Serialized response
		try: compressed = zlib.compress(jsonSerial, 1)
		except zlib.error:
