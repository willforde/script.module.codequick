...
class CacheAdapter(HTTPAdapter):
	...
	*args, **kwargs --- Any or all keywords that are accepted by requests.adapters.HTTPAdapter
	...
	def __del__(self):
		...
		try: lastTime = float(lastTime) + 1209600 # Every 14 Days
	...
	def build_response(self, request, response, from_cache=False):
		...
		if from_cache is False and request.method == "GET":
			...
			# Cache any cachable response
				...
				if response.chunked: body = "".join([chunk for chunk in response.stream(decode_content=False)])
				else: body = response.read(decode_content=False)
...
class CacheHandler(object):
	...
	def deserialize(self):
		...
		try:
			...
			cached["headers"] = {key: unicode(base64.b64decode(str(value))) for key, value in cached["headers"].iteritems()}
