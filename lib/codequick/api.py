...
def _translatePath(path):
	...
	if path[:10] == "special://": return unicode(xbmc.translatePath(path), "utf8")
	else: return unicode(path, "utf8")
...
def cls_for_route(route, raise_on_error=False):
	...
	for cls, pattern in _routes.iteritems():
		...
		if raise_on_error: raise KeyError("No class for route: %s" % route)
		else: return None
...
def run(buggalo_email=None, debug=False, error_time=10000):
	...
	try:
		...
		argv = sys.argv
		if argv[0][:9] == "plugin://": url = argv[0] + argv[2]
		else:
			argvLen = len(argv)
			if   argvLen == 1: raise ValueError("No action value was giving from script call")
			elif argvLen == 2: url = "plugin://%s/%s" % (str(addonID), argv[1])
			else:
		...
		logging.logger.debug('Dispatching to route "%s": class "%s"', route, cls.__name__)
		...
		if urlObject.query:
			...
			logging.logger.debug("Called with args: %r", _args)
		else:
			Base._args = {}
	...
	# Handle any exception with the buggalo module
		...
		logging.logger.debug("# Total time to execute: %s", time.time() - before)


class Base(collections.MutableMapping, logging.logger):
	...
	def url_for_current(cls, params=None, *querys_as_string):
		...
		return urlparse.urlunsplit((_urlObject.scheme, _urlObject.netloc, _urlObject.path, "&".join(querys_as_string), ""))
	...
	def urlencode(query):
		...
		return "&".join([_str(key) + "=" + (_quote_plus(value.encode("utf8")) if _isinstance(value, _unicode) else _quote_plus(str(value))) for key, value in query.iteritems()])
	...
	def getLocalizedString(id):
		...
		if id >= 30000 and id <= 30999: return addonData.getLocalizedString(id)
		elif id >= 32000 and id <= 32999: return scriptData.getLocalizedString(id)
		else: return xbmc.getLocalizedString(id)
	...
	def getSetting(id, raw_setting=False):
		...
		if raw_setting: return unicode(setting, "utf8")
		elif setting.isdigit(): return int(setting)
		elif setting == "true": return True
		elif setting == "false": return False
		else: return unicode(setting, "utf8")
	...
	def fanart(self):
		...
		if self.__fanart is not None: return self.__fanart
		else:
	...
	def path(self):
		...
		if self.__path is not None: return self.__path
		else:
	...
	def profile(self):
		...
		if self.__profile is not None: return self.__profile
		else:
	...
	def _path_global(self):
		...
		if self.__path_global is not None: return self.__path_global
		else:
	...
	def _profile_global(self):
		...
		if self.__profile_global is not None: return self.__profile_global
		else:
	...
	def utils(self):
		...
		if self.__utils: return self.__utils
		else:
			from . import utils
			self.__utils = utils
			return utils
	...
	def requests(self, max_age=60, max_retries=0):
		...
		"""
		if self.__session is not None: return self.__session
		else:
			...
			# Create a caching request session
				...
				session = requests_caching.cache_session(self.profile, 0 if u"refresh" in self else max_age, max_retries)
